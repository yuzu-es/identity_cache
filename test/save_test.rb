...
class SaveTest < IdentityCache::TestCase
  ...
  def test_create
    ...
    IdentityCache::CacheEntry.expects(:delete).with("#{NAMESPACE}index:Record:id/title:#{cache_hash('2/bob')}")
    IdentityCache::CacheEntry.expects(:delete).with("#{NAMESPACE}index:Record:title:#{cache_hash('bob')}")
    IdentityCache::CacheEntry.expects(:delete).with("#{NAMESPACE}blob:Record:#{cache_hash("created_at:datetime,id:integer,record_id:integer,title:string,updated_at:datetime")}:2").once
    @record.save
  end

  def test_update
    # Regular flow, write index id, write index id/tile, delete data blob since Record has changed
    IdentityCache::CacheEntry.expects(:delete).with("#{NAMESPACE}index:Record:id/title:#{cache_hash('1/fred')}")
    IdentityCache::CacheEntry.expects(:delete).with("#{NAMESPACE}index:Record:title:#{cache_hash('fred')}")
    IdentityCache::CacheEntry.expects(:delete).with(@blob_key)

    # Delete index id, delete index id/title
    IdentityCache::CacheEntry.expects(:delete).with("#{NAMESPACE}index:Record:id/title:#{cache_hash('1/bob')}")
    IdentityCache::CacheEntry.expects(:delete).with("#{NAMESPACE}index:Record:title:#{cache_hash('bob')}")
